...
$page_title = "Laporan Usulan Pensiun";
...
$filter_tahun  = isset($_GET['tahun'])  ? $_GET['tahun']  : $default_tahun;
...
$query .= " ORDER BY p.tanggal_pensiun ASC, p.created_at DESC";
...
if (!$stats) {
    $stats = ['total'=>0,'diajukan'=>0,'disetujui'=>0,'ditolak'=>0,'belum_diajukan'=>0];
}

// Daftar tahun untuk filter
$tahun_query  = "SELECT DISTINCT YEAR(tanggal_usulan) as tahun FROM usulan_pensiun WHERE tanggal_usulan IS NOT NULL ORDER BY tahun DESC";
$tahun_result = $koneksi->query($tahun_query);
$tahun_list   = [];
if ($tahun_result && $tahun_result->num_rows > 0) {
    while ($t = $tahun_result->fetch_assoc()) $tahun_list[] = $t['tahun'];
}
if (empty($tahun_list)) {
    $cy = date('Y');
    for ($i = 0; $i < 5; $i++) $tahun_list[] = $cy - $i;
}
...
<head>
    ...
    <style>
        ...
        .page-header .logo-image { width: 200%; height: 200%; object-fit: contain; }
        .page-header .header-text h1 { color: white; margin: 0; font-size: 2rem; font-weight: bold; }
        .page-header .page-subtitle { color: rgba(255,255,255,0.9); margin: 5px 0 0 0; font-size: 0.95rem; }
        .page-header .d-flex { display: flex; }
        .page-header .align-items-center { align-items: center; }
        .page-header .me-4 { margin-right: 1.5rem; }
        ...
        .stat-card.diajukan  { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); }
        .stat-card.disetujui { background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); }
        .stat-card.ditolak   { background: linear-gradient(135deg, #fa709a 0%, #fee140 100%); }
        .stat-card.belum     { background: linear-gradient(135deg, #a8edea 0%, #fed6e3 100%); color: #333; }
        .stat-card h3 { font-size: 2.5rem; margin: 0 0 5px 0; font-weight: bold; }
        ...
        .filter-section .row { display: flex; gap: 15px; align-items: flex-end; flex-wrap: wrap; }
        .filter-section .form-group { flex: 1; min-width: 180px; }
        .filter-section .form-group label { display: block; margin-bottom: 5px; font-weight: 500; color: #333; }
        .filter-section .form-control { width: 100%; padding: 8px 12px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; }

        .btn-export {
            background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%);
            color: white; border: none; padding: 10px 20px; border-radius: 5px;
            text-decoration: none; display: inline-flex; align-items: center; gap: 8px;
            transition: all 0.3s; font-size: 14px; cursor: pointer;
        }
        .btn-export:hover { transform: translateY(-2px); box-shadow: 0 4px 8px rgba(0,0,0,0.2); color: white; text-decoration: none; }
        .btn-export.btn-danger  { background: linear-gradient(135deg, #ee0979 0%, #ff6a00 100%); }
        .btn-export.btn-primary { background: linear-gradient(135deg, #2c3e50 0%, #34495e 100%); }

        .export-buttons { display: flex; gap: 10px; justify-content: flex-end; margin-bottom: 20px; flex-wrap: wrap; }
        ...
        .table thead { background: linear-gradient(135deg, #2c3e50 0%, #34495e 100%); color: white; }
        .table thead th { padding: 15px 10px; text-align: left; font-weight: 600; font-size: 14px; white-space: nowrap; }
        .table tbody td { padding: 12px 10px; border-bottom: 1px solid #f0f0f0; font-size: 14px; vertical-align: middle; }
        ...
        /* Pensiun mendekati: highlight baris */
        .row-soon { background: #fff8e1 !important; }
        .badge-pensiun { background: #ff9800; color: white; padding: 3px 8px; border-radius: 10px; font-size: 11px; }
    </style>
</head>
<body>
<main class="main-content">
    ...
    <div class="page-header">
        <div class="d-flex align-items-center">
            ...
            <div class="header-text">
                <h1><i class="fas fa-user-graduate me-3"></i><?= $page_title ?></h1>
                <p class="page-subtitle">Laporan dan statistik usulan pensiun pegawai</p>
            </div>
        </div>
    </div>
    ...
    <div class="export-buttons">
        <a href="export_laporan_pensiun.php?tahun=<?= $filter_tahun ?>&status=<?= $filter_status ?>&search=<?= urlencode($search) ?>"
           class="btn-export" target="_blank">
            <i class="fas fa-file-excel"></i> Export Excel
        </a>
        <a href="export_laporan_pensiun_pdf.php?tahun=<?= $filter_tahun ?>&status=<?= $filter_status ?>&search=<?= urlencode($search) ?>"
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    if ($result && $result->num_rows > 0):
                        while ($row = $result->fetch_assoc()):
                            ...
                            // Highlight jika pensiun dalam 3 bulan ke depan
                            $tgl_pensiun = strtotime($row['tanggal_pensiun']);
                            $bulan_lagi  = ($tgl_pensiun - time()) / (30 * 24 * 3600);
                            $row_class   = ($bulan_lagi <= 3 && $bulan_lagi >= 0) ? 'row-soon' : '';
                    ?>
                        <tr class="<?= $row_class ?>">
                            <td><?= $no++ ?></td>
                            <td>
                                <strong><?= htmlspecialchars($row['nomor_usulan']) ?></strong><br>
                                <small class="text-muted"><?= date('d/m/Y', strtotime($row['tanggal_usulan'])) ?></small>
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                <small class="text-muted"><?= htmlspecialchars($row['golongan']) ?></small>
                            </td>
                            <td><?= htmlspecialchars($row['jabatan_terakhir']) ?></td>
                            <td>
                                <span style="background:#6c757d;color:white;padding:4px 8px;border-radius:4px;font-size:11px;">
                                    <?= htmlspecialchars(strtoupper($row['jenis_pensiun'] ?: 'BUP')) ?>
                                </span>
                            </td>
                            <td>
                                <?= date('d/m/Y', strtotime($row['tanggal_pensiun'])) ?>
                                <?php if ($bulan_lagi <= 3 && $bulan_lagi >= 0): ?>
                                    <br><span class="badge-pensiun"><i class="fas fa-exclamation"></i> Segera</span>
                                <?php endif; ?>
                            </td>
                            <td>
                            ...
                            <td colspan="8">
                                <div class="empty-state">
                                    ...
                                    <small class="text-muted">
                                        ...
                                        <?php else: ?>
                                            Belum ada data usulan pensiun pada tahun <?= $filter_tahun ?>
                                        <?php endif; ?>
                                    </small><br><br>
                ...
                <div style="margin-top:20px;padding:15px 20px;background:linear-gradient(135deg,#2c3e50 0%,#34495e 100%);border-radius:8px;color:white;">
                    <p style="margin:0;font-size:14px;">
                        ...
                        <strong><?= number_format($stats['total']) ?></strong> usulan pensiun tahun <strong><?= $filter_tahun ?></strong>
                    </p>
                </div>
            </div>
        </div>
